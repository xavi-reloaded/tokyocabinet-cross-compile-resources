...
//  if(inet_aton(addr, &sain.sin_addr) == 0) return -1;
  if ((sain.sin_addr.S_un.S_addr = inet_addr(addr)) == -1) return -1;
  uint16_t snum = port;
...
//       errno != ETIMEDOUT) break;
    if (dwError != WSAEINPROGRESS && dwError != WSAEWOULDBLOCK) break;
  } while(tctime() <= dl);
  close(fd);
  return -1;
}
...
//  if(inet_aton(addr ? addr : "0.0.0.0", &sain.sin_addr) == 0) return -1;
  if ((sain.sin_addr.S_un.S_addr = inet_addr(addr ? addr : "0.0.0.0")) == -1) return -1;
  uint16_t snum = port;
  sain.sin_port = htons(snum);
  int fd = socket(PF_INET, SOCK_STREAM, 0);
  if(fd == -1) return -1;
...
//  if(shutdown(fd, 2) != 0 && errno != ENOTCONN && errno != ECONNRESET) err = true;
    ...
    {
      DWORD dwError = WSAGetLastError();
      if (dwError != WSAENOTCONN) err = true;
    }
...
//  } while((errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK) && tctime() <= sock->dl);
    } while ((dwError == WSAEINTR) || (dwError == WSAEWOULDBLOCK) && tctime() <= sock->dl);
  sock->end = true;
  return -1;
}
