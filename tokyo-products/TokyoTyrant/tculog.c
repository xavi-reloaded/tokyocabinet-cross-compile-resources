...
bool tculogadbsync(TCULOG *ulog, uint32_t sid, TCADB *adb){
  assert(ulog && adb);
  bool err = false;
...
bool tculogadboptimize(TCULOG *ulog, uint32_t sid, TCADB *adb, const char *params){
  assert(ulog && adb);
  bool err = false;
  bool dolog = tculogbegin(ulog, -1);
  ...
  if(tculogopen(sulog, path, 0)){
    ...
    if(ulrd){
      ...
      while((rbuf = tculrdread(ulrd, &rsiz, &rts, &rsid)) != NULL){
        bool cc;
        if(!tculogadbredo(adb, rbuf, rsiz, ulog, rsid, &cc) || (con && !cc)){
          err = true;
          break;
        }
      }
...
bool tculogadbredo(TCADB *adb, const char *ptr, int size, TCULOG *ulog, uint32_t sid, bool *cp){
  ...
  *cp = true;
  ...
  case TTCMDPUT:
    if(size >= sizeof(uint32_t) * 2){
      ...
      vsiz = TTNTOHL(vsiz);
      rp += sizeof(vsiz);
      if(tculogadbput(ulog, sid, adb, rp, ksiz, rp + ksiz, vsiz) != exp) *cp = false;
    } else {
      err = true;
    }
    break;
  case TTCMDPUTKEEP:
    if(size >= sizeof(uint32_t) * 2){
      ...
      vsiz = TTNTOHL(vsiz);
      rp += sizeof(vsiz);
      if(tculogadbputkeep(ulog, sid, adb, rp, ksiz, rp + ksiz, vsiz) != exp) *cp = false;
    } else {
      err = true;
    }
    break;
  case TTCMDPUTCAT:
    if(size >= sizeof(uint32_t) * 2){
      ...
      vsiz = TTNTOHL(vsiz);
      rp += sizeof(vsiz);
      if(tculogadbputcat(ulog, sid, adb, rp, ksiz, rp + ksiz, vsiz) != exp) *cp = false;
    } else {
      err = true;
    }
    break;
  case TTCMDOUT:
    if(size >= sizeof(uint32_t)){
      ...
      ksiz = TTNTOHL(ksiz);
      rp += sizeof(ksiz);
      if(tculogadbout(ulog, sid, adb, rp, ksiz) != exp) *cp = false;
    } else {
      err = true;
    }
    break;
  ...
  case TTCMDSYNC:
    if(size == 0){
      if(tculogadbsync(ulog, sid, adb) != exp) *cp = false;
    } else {
      err = true;
    }
    break;
  case TTCMDOPTIMIZE:
    if(size >= sizeof(uint32_t)){
      ...
      if(tculogadboptimize(ulog, sid, adb, params) != exp) *cp = false;
      tcfree(params);
    } else {
      err = true;
    }
  ...
  case TTCMDVANISH:
    if(size == 0){
      if(tculogadbvanish(ulog, sid, adb) != exp) *cp = false;
    } else {
      err = true;
    }
    break;
